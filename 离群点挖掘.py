...
def plot_lof(result, method):
    ...
    plt.scatter(result[result['local outlier factor'] <= method].index,result[result['local outlier factor'] <= method]['local outlier factor'], c='black', s=50,
...
# 根据文件位置自行修改
posi = pd.read_excel(r'D:\Python\云计算课设\已结束项目任务数据.xls') #注意输入文件的具体地址
lon = np.array(posi["任务gps经度"][:]) # 注意excel文档中经度一栏的格式，gps与经度之间没有空格
lat = np.array(posi["任务gps 纬度"][:]) # 纬度
A = list(zip(lat, lon)) # 按照纬度-经度匹配
# 获取任务密度，取第5 邻域，阈值为2（LOF 大于2 认为是离群值）
