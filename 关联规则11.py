...
class Apriori(object):
    ...
    def __candidate_itemsets_lk(self):
        ''' 根据 L1 创建多项频繁项集 Lk ，非频繁项集的任何超集都不是频繁项集 '''
    ...
    def __confidence_selects(self):
        ''' 生成关联规则，其中 support_selects 已经按照长度大小排列 '''
            ...
            for recommend_len in range(1, len(groups_list)):
                for recommend in itertools.combinations(groups_list, recommend_len):
                    ...
                    if Conf >= self.minConf:
                        ...
                        self.confidence_select[items].setdefault(','.join(recommend),{'Support': Supp_groups,
'Confidence': Conf})
    ...
    def __creat_Recommend_list(self,start_list,end_list,itemSet):
        ...
        if set(end_list).issubset(itemSet):
            ...
            self.recommend_selects[start_str].setdefault(end_str, {'Support': self.apriori_rules.loc[(start_str,
end_str), 'Support'], 'Confidence': self.apriori_rules.loc[(start_str, end_str), 'Confidence']})
